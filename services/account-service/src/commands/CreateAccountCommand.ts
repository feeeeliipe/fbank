...
export class CreateAccountCommand {
    ...
    async execute(params: ICreatedAccountCommandParams) {
        ...
        const event: EventDriveModel.CreatedAccountEvent = {
            id: account.id,
            customerName: account.customerName,
            customerWage: account.customerWage
        }
        await this.snsPublisher.publish(`${process.env.ACCOUNT_SNS}`, EventDriveModel.AccountEvents.ACCOUNT_CREATED, event);
        return { 
            id: account.id,
            customerName: account.customerName,
            customerWage: account.customerWage
